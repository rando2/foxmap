...
with open('/home/rando2/genome/RACA/rec_chrs.canFam3.segments.formatted.csv','rb') as csvfile:
    ...
    for line in data:
        ...
        if raca not in racadog.keys():
            ...
            #if ddir != racadog[raca][2] and int(dhigh)-int(dlow) > racadog[raca][1]-racadog[raca][0]:
            #    print line
            #    racadog[raca][2] = ddir
            #    print racadog[raca]
            #if dlow < racadog[raca][0]:
            #    racadog[raca][0] = int(dlow)
            #if dhigh > racadog[raca][1]:
            #    racadog[raca][1] = int(dhigh)
